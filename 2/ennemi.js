...
class Ennemi{
    constructor(x, y, map, index, piege){
        ...
        this.piege = piege;
        this.cheminsInterdits = [];
        this.cheminPrecedant = "";
        this.cooldownCheminChange = 0;
    }


    CheminEnString(chemin){
        let string = "";
        for(let c in chemin){
    ...
    CalculerMouvement(intelligence, joueurX, joueurY, map, ennemis){
        ...
        if(intelligence > 1){
            ...
            this.maxLongeurChemin = 0;
            this.coutParIntersection = 50;
            this.poidsDuCout = 10;

            switch(intelligence){
                case 2: this.maxLongeurChemin = 8; break;
                case 3:
                    this.maxLongeurChemin = 32;
            ...
            if(this.cooldownCheminChange < 0){
                ...
                for(let c in this.chemins){
                    ...
                    if(scoreChemin < plusPetit){ 
                        if(!this.cheminsInterdits.includes(this.CheminEnString(ch.coords))){
                            choixPrefere = c; plusPetit = scoreChemin;
                        }
                    }
                }
                this.chemin = this.chemins[choixPrefere].coords;
                let cheminString = this.CheminEnString(this.chemin);

                if(this.cheminPrecedant != cheminString){
                    if(this.cheminsInterdits.push(this.cheminPrecedant) > 2){
                        this.cheminsInterdits.shift();
                    }
                    this.cheminPrecedant = cheminString;
                    this.cooldownCheminChange = GetRandomInt(8, 24);
                }
            }
            ...
            if(this.chemin.length > 1){
                ...
                if(this.chemin.length > 2){
                    if(map.GetTileType(this.chemin[0]) == "-" && map.GetTileType(this.chemin[1]) == "-" && map.GetTileType(this.chemin[2]) == " "){
                        this.bas = true;
                    }
                }
            }
        }
        if(intelligence == 1 || this.chemin.length == 1){
            // Simple déplacement
            this.intelligence = 1;
    ...
    Pathfind(map, ennemis, posJoueur, indexChemin){
        ...
        for(let e in ennemis){
            let en = ennemis[e];
            if(coordCible[0] == en.GetPosTileX() / 16 && coordCible[1] == en.GetPosTileY() / 16){
                cout += this.coutParIntersection;
            }
        }

        switch (map.GetTileType(coordCible)){
            // ===== DANS LE VIDE =====
            case " ":
                switch (map.GetTileType(coordBas)){
                    // ON TOMBE
                    case " ":
                        ...
                        while(continu){
                            ...
                            if(map.GetTileType(bas) == " "){
                                coutOffset += 2;
                                tempCoords.push(bas);
                            }
                            else{
                                if(!tempCoords.some(c => c[0] == cible[0] && c[1] == cible[1])){
                    ...
                    case "O":
                        this.PathfindDeplacementHorizontal(map, ennemis,chemin, posJoueur, coordDroite, coordGauche);
                        break;
            ...
            case "H":
                ...
                if(!coords.some(c => c[0] == coordHaut[0] && c[1] == coordHaut[1])){
                    switch(map.GetTileType(coordHaut)){
                        case "O": break;
                        default:
    ...
    PathfindDeplacementHorizontal(map, ennemis, chemin, posJoueur, coordDroite, coordGauche){
        let tempCout;
        if(!chemin.coords.some(c => c[0] == coordDroite[0] && c[1] == coordDroite[1])){
            tempCout = 4;
            switch(map.GetTileType(coordDroite)){
                case "O": break;
                case "-": tempCout = 5;
                case " ":
                case "H":
                    this.chemins.push({"cout": chemin.cout + tempCout, "distance": this.Distance(coordDroite, posJoueur), "coords": [...chemin.coords, coordDroite]});
                    this.Pathfind(map, ennemis, posJoueur, this.chemins.length - 1);
                    break;
            }
        }
        if(!chemin.coords.some(c => c[0] == coordGauche[0] && c[1] == coordGauche[1])){
            tempCout = 4;
            switch(map.GetTileType(coordGauche)){
                case "O": break;
                case "-": tempCout = 3;
                case " ":
                case "H":
                    this.chemins.push({"cout": chemin.cout + tempCout, "distance": this.Distance(coordGauche, posJoueur), "coords": [...chemin.coords, coordGauche]});
                    this.Pathfind(map, ennemis, posJoueur, this.chemins.length - 1);
                    break;
            }
        }
    ...
    Tick(map, ennemis){
        ...
        if(this.pause){
            ...
            // DESCENDRE ECHELLE
                ...
                let xDesire = this.x;
                if(!map.EstVide(ennemis, this.x + 8, this.y - 1)){
            ...
            // SUR LE SOL
                ...
                this.TickAtteri(dejaGrimpe, ennemis);
            }
            // SUR LA BARRE
            ...
            // TOMBER
                ...
                if(!map.EstVide(ennemis, this.x + 8, this.y)){
            ...
            this.CalculerPointsDeCollisionTerrain();
    ...
    CalculerPointsDeCollisionTerrain(){
